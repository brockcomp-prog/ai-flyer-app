...
export type Club = 
  'None'
  // Club / Nightlife
  | 'Nightclub' | 'DJ Set' | 'Live Band' | 'Lounge' | 'Concert' | 'Rave' | 'Afterparty';
...
export type FlyerSize = 'square' | 'portrait' | 'landscape';
...
export interface MonetizedFlyerOutput {
  cleanFlyerImageUrl: string;
  watermarkedFlyerImageUrl: string;
}

// FIX: Add the missing StoredFlyer interface, which is used in the LibraryModal component.
export interface StoredFlyer extends MonetizedFlyerOutput {
  id: string;
  createdAt: number;
}
